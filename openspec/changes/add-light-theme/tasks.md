...
- [ ] 1.6 Run `npm run build` and confirm the SCSS compiles with no errors and `dist/ui-library.css` contains both `[data-theme='dark']` and `[data-theme='light']` rule blocks. **Blocked**: build compiles all SCSS together, so this can't pass until task groups 2–3 stop referencing the old unprefixed token names. Confirmed the current build failures are confined to not-yet-converted files (`variables.module.scss`, `Tabs.module.scss`, `TextArea.module.scss`, `Switch.module.scss`, `ColourPalettePicker.scss`); nothing errors in `_variables.scss` or `_core_theme.scss`. Revisit once groups 2–3 are done.
